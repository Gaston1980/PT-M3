...
async function problemB () {
  ...
  // AsyncAwait version
  // ['poem-two/stanza-01.txt', 'poem-two/stanza-02.txt', 'poem-two/stanza-03.txt', 'poem-two/stanza-04.txt', 'poem-two/stanza-05.txt', 'poem-two/stanza-06.txt', 'poem-two/stanza-07.txt', 'poem-two/stanza-08.txt']

  const promeses = filenames.map(f => promisifiedReadFile(f))
  //[p1, p2, p3, p4, etc]

  //response es un array con las stanzas
  const response = await Promise.all(promeses)
...
async function problemD () {
  ...
  // AsyncAwait version
  try { // ¿Qué quiero hacer?
    for(const file of filenames) {
      blue(await promisifiedReadFile(file))
    }
  } catch (error) { //¿Qué hago si hay un error en la ejecución del try?
    magenta(error)
  } finally { // Quiero que siempre ocurra esto:
    console.log('done')
  }
}
